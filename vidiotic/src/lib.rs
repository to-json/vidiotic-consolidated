...
/// that drives it: the [`commands`] vocabulary, the [`grammar`] that produces
...
///
/// Re-exported so `crate::render::…`, `crate::grammar::…` and the rest are
...
pub use vidiotic_play::{clock, commands, gfx, grammar, render, sequencer, shader, undo};
