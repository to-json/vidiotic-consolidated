...
fn header(ui: &mut Ui) {
    ui.horizontal(|ui| {
        ...
        ui.label(
            egui::RichText::new("fs · 11 inputs")
                .small()
                .color(FG_MUTED),
        );
    });
...
fn slider(ui: &mut Ui, i: usize, min: f32, max: f32, v: &mut f32) -> Rect {
    let (rect, resp) = ui.allocate_exact_size(vec2(CTL_W - 52.0, 22.0), Sense::click_and_drag());
...
fn event_button(ui: &mut Ui, at: &mut f64, time: f64) -> Rect {
    ...
    if flash > 0.0 {
        painter.rect_filled(
            rect,
            CornerRadius::same(4),
            alpha(ACCENT, (flash * 130.0) as u8),
        );
    }
    painter.text(
        rect.center(),
...
fn pad(ui: &mut Ui, min: [f32; 2], max: [f32; 2], p: &mut [f32; 2]) -> Rect {
    ...
    for k in 1..4 {
        ...
        painter.line_segment(
            ...
            Stroke::new(1.0, alpha(BORDER, 90)),
        );
        painter.line_segment(
    ...
    painter.line_segment(
        [Pos2::new(dot.x, rect.min.y), Pos2::new(dot.x, rect.max.y)],
        Stroke::new(1.0, ACCENT_DIM),
    );
    painter.line_segment(
        [Pos2::new(rect.min.x, dot.y), Pos2::new(rect.max.x, dot.y)],
        Stroke::new(1.0, ACCENT_DIM),
    );
...
fn color_row(ui: &mut Ui, i: usize, c: &mut Hsva) -> Rect {
    ...
    ui.painter()
        .rect_filled(swatch, CornerRadius::same(4), Color32::from(*c));
    ...
    for (k, label) in ["H", "S", "V"].iter().enumerate() {
        ...
        let track = Rect::from_min_max(
            Pos2::new(rect.min.x + 12.0, rect.center().y - 1.0),
            Pos2::new(rect.max.x, rect.center().y + 1.0),
        );
...
fn fft_bars(ui: &mut Ui, time: f64) -> Rect {
    ...
    for k in 0..n {
        ...
        painter.rect_filled(
            bar,
            CornerRadius::ZERO,
            alpha(ACCENT, 90 + (mag * 165.0) as u8),
        );
    }
    rect
}
...
pub fn show_widgets(ui: &mut Ui, st: &mut DemoState) {
    ...
    ui.horizontal(|ui| {
        ...
        .enumerate()
        {
            ...
            if flash > 0.0 {
                painter.rect_filled(
                    rect,
                    CornerRadius::same(4),
                    alpha(ACCENT, (flash * 120.0) as u8),
                );
            }
            painter.text(
                rect.center(),
        ...
        for k in 0..4 {
            ...
            if k == beat {
                ui.painter()
                    .circle_filled(center, 4.0 + pulse * 1.5, ACCENT);
            } else {
...
fn wsegmented(ui: &mut Ui, salt: &str, labels: &[&str], selected: usize) {
    ui.scope(|ui| {
        ...
        for (i, label) in labels.iter().enumerate() {
            let galley =
                ui.painter()
                    .layout_no_wrap((*label).into(), FontId::proportional(11.0), FG);
            let (rect, _) =
                ui.allocate_exact_size(galley.size() + vec2(16.0, 10.0), Sense::hover());
...
fn wchip(ui: &mut Ui, text: &str, tint: Option<Color32>, removable: bool) {
    ...
    let (rect, resp) =
        ui.allocate_exact_size(vec2(galley.size().x + 12.0 + close_w, 18.0), Sense::click());
    let fill = if resp.hovered() {
        fill.gamma_multiply(1.25)
    } else {
        fill
    };
...
fn wtile(ui: &mut Ui, clip: &crate::schema::MockClip, pulse: f32, _time: f64) {
    ...
    for k in 0..6 {
        ...
        painter.rect_filled(
            band,
            CornerRadius::ZERO,
            util::hsl(190.0 + h0 * 130.0, 0.35, 0.12 + h1 * 0.22),
        );
    }
    if resp.hovered() {
    ...
    if clip.selected {
        ...
        painter.rect_stroke(
            rect,
            radius,
            Stroke::new(1.0, Color32::from_rgb(255, 171, 64)),
            StrokeKind::Inside,
        );
    } else if resp.hovered() {
    ...
    if clip.role == MockRole::Playing {
        ...
        painter.rect_stroke(
            rect,
            radius,
            Stroke::new(2.0, alpha(Color32::from_rgb(98, 217, 107), a)),
            StrokeKind::Inside,
        );
    }
}
