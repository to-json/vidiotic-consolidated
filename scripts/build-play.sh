...
echo "building vidiotic-play ($PROFILE) for wasm32..."
...
cargo build -p vidiotic-play --target wasm32-unknown-unknown --no-default-features $CARGO_PROFILE_FLAG || exit 1

WASM="target/wasm32-unknown-unknown/$PROFILE/vidiotic_play.wasm"
[ -f "$WASM" ] || { echo "no wasm at $WASM" >&2; exit 1; }

echo "running wasm-bindgen..."
wasm-bindgen --target web --out-dir web/pkg --no-typescript "$WASM" || exit 1

OUT=web/pkg/vidiotic_play_bg.wasm
...
echo "serve:  python3 -m http.server -d web 8080   then open http://localhost:8080"
