#!/usr/bin/env bash
# build-wasm — compile one web app to wasm and emit the loadable bundle into web/.
#
# One script for both pages. `build-play.sh` and `build-chop.sh` were 110 of 119
# lines byte-identical: same toolchain checks, same version pin, same wasm-opt
# invocation with the same six `--enable-*` flags and the same explanation of why
# each is there. Two copies of that means the next thing learned about binaryen
# gets written into one of them. Those two names still exist as wrappers, because
# the docs, the smoke scripts and half the error messages in this repo name them.
#
...
#
# Usage:  scripts/build-wasm.sh <play|chop> [--debug] [--serve]
...
shift 2>/dev/null
case "$APP" in
  # crate, wasm/module stem, wasm-bindgen out-dir, and the page a browser opens.
  play) CRATE=vidiotic-play; STEM=vidiotic_play; PKG=web/pkg;      PAGE= ;;
  chop) CRATE=vidiotic-chop; STEM=vidiotic_chop; PKG=web/pkg-chop; PAGE=/chop.html ;;
  *) echo "usage: scripts/build-wasm.sh <play|chop> [--debug] [--serve]" >&2; exit 2 ;;
esac
...
echo "building $CRATE ($PROFILE) for wasm32..."
...
cargo build -p "$CRATE" --target wasm32-unknown-unknown --no-default-features $CARGO_PROFILE_FLAG || exit 1

WASM="target/wasm32-unknown-unknown/$PROFILE/$STEM.wasm"
[ -f "$WASM" ] || { echo "no wasm at $WASM" >&2; exit 1; }

echo "running wasm-bindgen..."
wasm-bindgen --target web --out-dir "$PKG" --no-typescript "$WASM" || exit 1

OUT=$PKG/${STEM}_bg.wasm
...
echo "serve:  python3 -m http.server -d web 8080   then open http://localhost:8080$PAGE"

if [ "$SERVE" = 1 ]; then
  echo
  echo "serving http://localhost:8080$PAGE (ctrl-c to stop)"
